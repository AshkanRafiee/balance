...
public class CsvExportTest {
    ...
    @Test public void csv_emptyList_containsOnlyTheHeader() {
        String csv = CsvExport.csv(ctx, new ArrayList<>());
        assertEquals("bank,account,date,date_local,time,amount_rial,amount_display,currency",
            line(csv, 0));
    ...
    @Test public void row_oneTransaction_laysOutCellsInFixedOrder() {
        ...
        String row = line(CsvExport.csv(ctx, Arrays.asList(t)), 1);
    ...
    @Test public void csv_unsortedInputs_writesOldestFirst() {
        ...
        String csv = CsvExport.csv(ctx, txs);
    ...
    @Test public void csv_persianToman_formatsDisplayAndUnitInFarsi() {
        ...
        String row = line(CsvExport.csv(fa, Arrays.asList(t)), 1);
        assertTrue(row.contains("۱۲۵٬۰۰۰"));
        assertTrue(row.endsWith(",تومان"));
    }
}